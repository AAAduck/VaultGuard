...
- **并行化约束（v2.1）**：段与段彼此独立（各自 nonce/AAD），因此加密/认证/解密可跨线程并行；但**段序号必须与文件中的物理顺序一致**，所以并行只发生在计算阶段，写盘一律按序号顺序串行（`vgs2::write_segments_sequential`：占位段头 → 密文+tag → 统一回填段头 → 单次 fsync）。改这一块时不要打破「写盘顺序 = 序号顺序」，否则崩溃回退语义失效。
- **内存边界**：段目标 64MiB（`SEG_TARGET`）、在途明文预算 512MiB（`PARALLEL_BUF_BUDGET`）、单批上限 128MiB（`PARALLEL_MEM_CAP`）；超过单批上限的单个条目落盘为临时批并流式串行加密，绝不整段进内存。转发批默认驻留内存（避免多写/多擦一份明文临时文件）。worker 数 = `min(available_parallelism, 8)`。
